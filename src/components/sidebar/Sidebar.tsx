...
export default function Sidebar({ isOpen = true, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed lg:sticky top-14 z-30 h-[calc(100vh-3.5rem)] w-60 
      ...
      >
        <nav className="py-6 px-3">
          ...
          {docsNavigation.map((section, i) => (
            <SidebarItem key={section.slug} section={section} activeSlug={activeSlug} index={i} />
          ))}
        </nav>
      </aside>
    </>
...
function SidebarItem({ section, activeSlug, index }: { section: DocSection; activeSlug: string; index: number }) {
