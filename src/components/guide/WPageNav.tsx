...
interface WPageNavProps {
  prev?: { title: string; slug: string };
  next?: { title: string; slug: string };
}

export default function WPageNav({ prev, next }: WPageNavProps) {
  const href = (slug: string) =>
    slug === "" ? WALKTHROUGH_BASE : `${WALKTHROUGH_BASE}/${slug}`;

  return (
    <div className="flex items-center justify-between gap-4 mt-12 pt-6 border-t not-prose"
